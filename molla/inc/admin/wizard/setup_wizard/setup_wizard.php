	...
	*/
	class Molla_Setup_Wizard {
		...
		public function init_setup_wizard() {
			...
			if ( ( ! empty( $_GET['page'] ) && $this->page_slug === $_GET['page'] ) || ( wp_doing_ajax() && isset( $_REQUEST['action'] ) && 0 === strpos( $_REQUEST['action'], 'molla_' ) ) ) {
				require_once 'class-molla-demo-history.php';
				new Molla_Demo_History;
			}
		}
		...
		private function get_imported_post_terms() {
			global $wpdb, $molla_imported_terms, $molla_imported_posts;
			$molla_imported_posts = array();
			/* translators: demo */
			$imported_posts = $wpdb->get_results( $wpdb->prepare( "SELECT post_id, meta_value FROM {$wpdb->postmeta} WHERE meta_key = '_molla_demo' AND meta_value LIKE %s", $wpdb->esc_like( $this->demo ) . '#%' ) );
			foreach ( $imported_posts as $p ) {
				$arr = explode( '#', $p->meta_value );
				if ( 2 == count( $arr ) && (int) $arr[1] !== (int) $p->post_id ) {
					$molla_imported_posts[ (int) $arr[1] ] = (int) $p->post_id;
				}
			}

			$molla_imported_terms = array();
			/* translators: demo */
			$imported_terms = $wpdb->get_results( $wpdb->prepare( "SELECT term_id, meta_value FROM {$wpdb->termmeta} WHERE meta_key = '_molla_demo' AND meta_value LIKE %s", $wpdb->esc_like( $this->demo ) . '#%' ) );
			foreach ( $imported_terms as $t ) {
				$arr = explode( '#', $t->meta_value );
				if ( 2 == count( $arr ) && (int) $arr[1] !== (int) $t->term_id ) {
					$molla_imported_terms[ (int) $arr[1] ] = (int) $t->term_id;
				}
			}
		}
		...
		public function replace_post_ids( $matches ) {
			global $molla_imported_posts;
			$ids     = array_map( 'intval', explode( ',', str_replace( '"', '', $matches[2] ) ) );
			$new_ids = array();
			foreach ( $ids as $id ) {
				if ( isset( $molla_imported_posts[ $id ] ) ) {
					$new_ids[] = $molla_imported_posts[ $id ];
				} else {
					$new_ids[] = $id;
				}
			}
		...
		public function replace_term_ids( $matches ) {
			global $molla_imported_terms;
			$ids     = array_map( 'intval', explode( ',', str_replace( '"', '', $matches[2] ) ) );
			$new_ids = array();
			foreach ( $ids as $id ) {
				if ( isset( $molla_imported_terms[ $id ] ) ) {
					$new_ids[] = $molla_imported_terms[ $id ];
				} else {
					$new_ids[] = $id;
				}
			}
		...
		private function import_after_functions( $demo ) {
			...
			$args = array(
				'posts_per_page' => -1,
				'post_type'      => array( 'page', 'header', 'footer', 'sidebar', 'popup', 'product_layout' ),
				'post_status'    => 'publish',
				'meta_query'     => array(
					'relation' => 'AND',
					array(
						'meta_key' => '_molla_demo',
						'compare'  => 'EXISTS',
					),
					array(
			...
			if ( $post_query->have_posts() ) {
				foreach ( $post_query->posts as $post ) {
					...
					$new_content = preg_replace_callback( '/("ids":")([0-9,]+)(")/', array( $this, 'replace_term_ids' ), $new_content );
					$new_content = preg_replace_callback( '/("id":")(\d{1,5})(")/', array( $this, 'replace_post_ids' ), $new_content );
					$new_content = preg_replace_callback( '/("product":")(\d+)(")/', array( $this, 'replace_post_ids' ), $new_content );

					$new_content = preg_replace_callback( '/("category":")([0-9,]+)(")/', array( $this, 'replace_term_ids' ), $new_content );
					$new_content = preg_replace_callback( '/("brands":")([0-9,]+)(")/', array( $this, 'replace_term_ids' ), $new_content );
					update_post_meta( $post->ID, '_elementor_data', wp_slash( $new_content ) );
				}
			}

			// Update elementor setttings
			...
			do_action( 'molla_demo_imported', true );
			flush_rewrite_rules();
		}
