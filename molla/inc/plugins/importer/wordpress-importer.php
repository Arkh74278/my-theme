...
require MOLLA_PLUGINS . '/importer/parsers.php';
	...
	class WP_Import extends WP_Importer {
		...
		function handle_upload() {
			...
			if ( isset( $file['error'] ) ) {
				...
				printf( molla_strip_script_tags( esc_html__( 'The export file could not be found at %s. It is likely that this was caused by a permissions problem.', 'molla' ) ), esc_html( $file['file'] ) );
				echo '</p>';
				return false;
			}
			...
			foreach ( $this->posts as $post ) {
				...
				*/
				...
				if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
					...
					printf( esc_html__( '%1$s &#8220;%2$s&#8221; already exists.', 'molla' ), $post_type_object->labels->singular_name, esc_html( $post['post_title'] ) );
					echo '<br />';
